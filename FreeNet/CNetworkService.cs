...
namespace FreeNet
{
    ...
    {
		int connected_count;
		CListener client_listener; // 클라이언트의 접속을 받아들이기 위한 객체
		SocketAsyncEventArgsPool receive_event_args_pool; // 메시지 수신, 전송 시 필요한 객체
		SocketAsyncEventArgsPool send_event_args_pool;
		BufferManager buffer_manager; // 메시지 수신, 전송 시 닷넷 비동기 소켓에서 사용할 버퍼를 관리하는 객체

		public delegate void SessionHandler(CUserToken token); // 클라이언트 접속이 이루어졌을 때 호출되는 델리게이트
		...
		public void on_connect_completed(Socket socket, CUserToken token)
		{
			...
			receive_event_arg.SetBuffer(new byte[40000], 0, 40000);
			...
			send_event_arg.SetBuffer(new byte[40000], 0, 40000);
